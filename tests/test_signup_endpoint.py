...
def test_signup_creates_row(client):
    with patch("server.email_client.send_confirmation", return_value={"id": "msg"}):
        resp = client.post("/api/signup", json={"email": "alice@example.com"}, headers={"Accept": "application/json"})
...
def test_signup_dedup_returns_existing_id(client):
    with patch("server.email_client.send_confirmation", return_value={"id": "msg"}):
        ...
        r2 = client.post("/api/signup", json={"email": "bob@example.com"}, headers={"Accept": "application/json"})
...
def test_signup_rejects_missing_email(client):
    resp = client.post("/api/signup", json={}, headers={"Accept": "application/json"})
    assert resp.status_code == 400


def test_signup_rejects_empty_email(client):
    resp = client.post("/api/signup", json={"email": ""}, headers={"Accept": "application/json"})
    assert resp.status_code == 400


def test_signup_rejects_invalid_email(client):
    resp = client.post("/api/signup", json={"email": "not-an-email"}, headers={"Accept": "application/json"})
    assert resp.status_code == 400


def test_signup_normalizes_email_case_and_whitespace(client):
    with patch("server.email_client.send_confirmation", return_value={"id": "msg"}):
        client.post("/api/signup", json={"email": "  Carol@Example.com  "}, headers={"Accept": "application/json"})
...
def test_signup_triggers_confirmation_email(client):
    with patch("server.email_client.send_confirmation", return_value={"id": "msg"}) as mock_send:
        client.post("/api/signup", json={"email": "dave@example.com"}, headers={"Accept": "application/json"})
...
def test_signup_succeeds_even_if_email_fails(client):
    with patch("server.email_client.send_confirmation", return_value=None):
        resp = client.post("/api/signup", json={"email": "eve@example.com"}, headers={"Accept": "application/json"})
    assert resp.status_code == 200
    row = db.get_signup_by_email("eve@example.com")
    assert row is not None


def test_signup_rejects_email_over_254_chars(client):
    long_email = "x" * 250 + "@a.com"  # 256 chars
    resp = client.post("/api/signup", json={"email": long_email}, headers={"Accept": "application/json"})
    assert resp.status_code == 400
